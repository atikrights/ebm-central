...
class _JoinScreenState extends ConsumerState<JoinScreen>
  ...
  String _teamCode = '';
  String _inviterName = '';
  bool _isAdminInvite = false;   // true when an admin invites another admin as sub_admin
  bool _migrateData = false;     // user's choice: migrate their existing data?
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _inviterName   = data['inviter']?['name'] ?? '';
          _teamName      = data['team']?['name'] ?? '';
          _teamCode      = data['team']?['code'] ?? '';
          _isAdminInvite = _tokenRole == 'admin'; // admin invited as sub_admin
        });
    ...
    return _glassContainer(
      ...
      child: Column(
        ...
        children: [
          ...
          // Invitation Badge (now with team and inviter info)
          Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              color: Colors.greenAccent.withOpacity(isDark ? 0.07 : 0.05),
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(_tokenRole.toUpperCase(),
                          style: const TextStyle(color: Colors.black, fontSize: 9, fontWeight: FontWeight.bold)),
                    ),
                  ],
                ),
                if (_inviterName.isNotEmpty || _teamName.isNotEmpty) ...[  
                  const SizedBox(height: 10),
                  const Divider(height: 1, color: Colors.white10),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      Icon(Icons.person_outline, size: 13, color: isDark ? Colors.white38 : Colors.black38),
                      const SizedBox(width: 6),
                      Text('Invited by: ', style: GoogleFonts.inter(fontSize: 11, color: isDark ? Colors.white38 : Colors.black38)),
                      Text(_inviterName, style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.bold,
                  ...
                  if (_teamName.isNotEmpty) ...[  
                    ...
                    Row(
                      children: [
                        Icon(Icons.group_work_rounded, size: 13, color: Colors.blueAccent.withOpacity(0.7)),
                        const SizedBox(width: 6),
                        Text('Team: ', style: GoogleFonts.inter(fontSize: 11, color: isDark ? Colors.white38 : Colors.black38)),
                        Text(_teamName, style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.bold,
                            color: Colors.blueAccent)),
                        if (_teamCode.isNotEmpty) ...[  
                          const SizedBox(width: 8),
                          Container(
          ...
          // ── Data Migration Toggle (only for admin role invited as sub_admin) ──
          if (_isAdminInvite) ...[  
            Container(
              padding: const EdgeInsets.all(16),
              decoration: BoxDecoration(
                color: Colors.orangeAccent.withOpacity(isDark ? 0.07 : 0.05),
                borderRadius: BorderRadius.circular(14),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      const Icon(Icons.swap_horiz_rounded, color: Colors.orangeAccent, size: 16),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text('Data Migration',
                          style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.bold,
                              color: isDark ? Colors.white : Colors.black87)),
                      ),
                      Switch.adaptive(
                        value: _migrateData,
                        onChanged: (v) => setState(() => _migrateData = v),
                  ...
                  Text(
                    _migrateData
                        ? '✓ Your existing companies, projects and tasks will be transferred to ${_inviterName.isNotEmpty ? _inviterName : 'the team leader'}\'s account.'
                        : 'Your existing data will remain separate. You can migrate it later from settings.',
                    style: GoogleFonts.inter(
                        fontSize: 11, height: 1.5,
                        color: _migrateData ? Colors.orangeAccent.withOpacity(0.8) : (isDark ? Colors.white38 : Colors.black38)),
                  ),
                ],
              ),
            ),
