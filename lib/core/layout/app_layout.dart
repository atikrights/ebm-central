...
import '../../features/chat/presentation/chat_settings_screen.dart';
...
class AppLayout extends ConsumerStatefulWidget {
  ...
  const AppLayout({super.key, this.initialIndex = 1});
...
class _AppLayoutState extends ConsumerState<AppLayout> {
  ...
  final List<Widget> _screens = [
    ...
    const DepartmentScreen(), // Index 3
    const AddScreen(),        // Index 4
    const ChatListScreen(),   // Index 5
    ...
    const TeamScreen(view: 0), // Index 33: All Users
    const TeamScreen(view: 1), // Index 34: Add New
    const TeamScreen(view: 2), // Index 35: Work Tracking
    const CompanyScreen(), // Index 36: Workplace -> Companies
    const ProjectsScreen(), // Index 37: Workplace -> Projects
    const TasksScreen(), // Index 38: Workplace -> Tasks
    const AssetLibraryScreen(), // Index 39: Assets Library
    const Scaffold(body: Center(child: Text('Workplace Dashboard', style: TextStyle(color: Colors.white)))), // Index 40: Workplace Dashboard
    const TeamsScreen(), // Index 41: Teams Dashboard
    const TeamsControlScreen(), // Index 42
  ...
  Widget _buildDesktopLayout(bool isDark) {
    ...
    return Column(
      children: [
        // ── Global Mac Title Bar (ONE time, at app level, desktop OS only) ──
        // ── Global Mac Title Bar removed here — it is now handled globally in app_router.dart (ShellRoute) ──
              ...
              child: Column(
                children: [
                  ...
                  _buildExpandableSidebarItem(
                    ...
                    isDark: isDark,
                    subItems: [
                      _SubMenuItem(index: 39, label: 'Assets Library', icon: Icons.photo_library_rounded),
                      _SubMenuItem(index: 43, label: 'Icon Library', icon: Icons.insert_emoticon_rounded),
                    ],
                  ),
                  _sidebarItem(2, Icons.auto_awesome_motion, 'Board', isExpanded, isDark),
                  _buildExpandableSidebarItem(
                    index: 45, // Now clicking 'Chat' goes to Dashboard
